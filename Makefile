...
cx16_EMUCMD := x16emu -fsroot . -debug -run -prg
box16_EMUCMD := box16 -hypercall_path . -sym $(PROGRAM).cx16.lbl -run -prg
emu_prog     :=
ifeq ($(emu_prog),box16)
	cx16_EMUCMD = $(box16_EMUCMD)
endif
ifeq ($(EMUCMD),)
...
vpath %.png $(SRCDIR)/$(TARGETLIST) $(SRCDIR)
